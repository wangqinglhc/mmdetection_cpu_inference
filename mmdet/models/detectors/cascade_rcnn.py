...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        #img_meta = [[{'filename': 'GSV_TestSet_430/desert-poles.jpg', 'ori_shape': (320, 470, 3), 'img_shape': (800, 1175, 3), 'pad_shape': (800, 1184, 3), 'scale_factor': 2.5, 'flip': False, 'img_norm_cfg': {'mean': np.array([123.675, 116.28 , 103.53 ], dtype=np.float32), 'std': np.array([58.395, 57.12 , 57.375], dtype=np.float32), 'to_rgb': True}}]]
    ...
    def detect(self, img):
        ...
        ori_shape = (*img.size, 3) 
        
        factor = scale_factor(img.size, scale=(1333, 800))
        scaled_size = scale_size(img.size, factor)

        img = transform(img, scaled_size).unsqueeze(0)
        ...
        proposal_list = self.rpn_head.get_bboxes(*rpn_outs, [scaled_size], [factor], self.test_cfg.rpn)

        #print(proposal_list[0].size())


        img_shape =(*scaled_size, 3)

       
        scale_factor = factor

        #print(img_shape, ori_shape, scale_factor)
       ...
       # print(bbox_pred)
       # print(img_shape, scale_factor, rescale)
        det_bboxes, det_labels = self.bbox_head[-1].get_det_bboxes(
            rois,
            cls_score,
            bbox_pred,
        ...
        bbox_result = bbox2result(det_bboxes, det_labels,
                                  self.bbox_head[-1].num_classes)
        #print(self.bbox_head[-1].num_classes)
        ms_bbox_result['ensemble'] = bbox_result
        ...
        #print(bbox_result)
    #bbox_result = bbox_result[0].detach().numpy()
        bboxes = []
        for idx, bboxes_class in enumerate(bbox_result):
            if idx == 4:
                break
        #print(type(bboxes_class))
            if not isinstance(bboxes_class, np.ndarray):
                bboxes.append(np.array(bboxes_class.detach().numpy()))
            else:
                bboxes.append(bboxes_class)
